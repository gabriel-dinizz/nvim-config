...
-- Patch nvim-treesitter.parsers module after it loads
...
-- Also patch immediately in case plugins are already loaded
local ok, parsers = pcall(require, "nvim-treesitter.parsers")
if ok and not parsers.ft_to_lang then
  parsers.ft_to_lang = get_lang
end
...
if lspconfig_status then
  ...
  local on_attach = function(_, bufnr)
    ...
    keymap.set("n", "<leader>gi", vim.lsp.buf.implementation, vim.tbl_extend("force", opts, { desc = "Goto implementation" }))
    keymap.set("n", "<leader>gr", vim.lsp.buf.references, vim.tbl_extend("force", opts, { desc = "Show references" }))
    keymap.set("n", "<leader>ds", vim.lsp.buf.document_symbol, vim.tbl_extend("force", opts, { desc = "Document symbols" }))
    keymap.set("n", "<leader>ws", vim.lsp.buf.workspace_symbol, vim.tbl_extend("force", opts, { desc = "Workspace symbols" }))
    keymap.set("n", "<leader>dws", vim.lsp.buf.dynamic_workspace_symbol or vim.lsp.buf.workspace_symbol, vim.tbl_extend("force", opts, { desc = "Dynamic workspace symbols" }))
    ...
    keymap.set("n", "<leader>d", vim.diagnostic.open_float, vim.tbl_extend("force", opts, { desc = "Show diagnostics" }))
